...
export const BASE_HIRAGANA: KanaItem[] = [
...
export const DAKUTEN_HIRAGANA: KanaItem[] = [
...
export const DAKUTEN_KATAKANA: KanaItem[] = [
...
export const HANDAKUTEN_KATAKANA: KanaItem[] = [
  { char: 'パ', romaji: 'pa' }, { char: 'ピ', romaji: 'pi' }, { char: 'プ', romaji: 'pu' },
  { char: 'ペ', romaji: 'pe' }, { char: 'ポ', romaji: 'po' },
]

export const HIRAGANA = BASE_HIRAGANA
export const KATAKANA = BASE_KATAKANA

export function buildKanaQuiz(items: KanaItem[], count: number): Array<{ target: KanaItem; options: string[] }> {
  ...
  return shuffled.map(target => {
    const distractors = items
      ...
      .slice(0, 3)
      .map(k => k.romaji)
    const options = [...distractors, target.romaji].sort(() => Math.random() - 0.5)
